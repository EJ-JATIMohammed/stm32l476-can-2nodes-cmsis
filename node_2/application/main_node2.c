...
void leds_init(void)
{
	...
	GPIOB->MODER &= ~(GPIO_MODER_MODE11 | GPIO_MODER_MODE12 | GPIO_MODER_MODE13 | GPIO_MODER_MODE14 | GPIO_MODER_MODE15);	    
	GPIOB->MODER |= (GPIO_MODER_MODE11_0 | GPIO_MODER_MODE12_0 | GPIO_MODER_MODE13_0 | GPIO_MODER_MODE14_0 | GPIO_MODER_MODE15_0);                             	                            
    
	GPIOB->ODR = (GPIOB->ODR & ~0x0000F800U)|((leds_data[0] & 0x1FU)<< GPIO_ODR_OD11_Pos);
}

void CAN1_RX0_IRQHandler(void)
{
	can_receive(CAN1, 0, true, &rx_id, &rx_ext, &rx_rtr, &rx_fmi, &rx_len, rx_data);
	GPIOB->ODR = (GPIOB->ODR & ~0x0000F800U)|(((*rx_data) & 0x1FU) << GPIO_ODR_OD11_Pos);
}
